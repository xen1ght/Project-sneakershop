document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("📦 Данные для отправки:", data);
    ...
    try {
      const res = await fetch("http://localhost:8080/api/sneakers/add", {
        method: "POST",
    ...
    try {
      ...
      document.querySelectorAll(".update-status").forEach(btn => {
        btn.addEventListener("click", async () => {
          ...
          try {
            const response = await fetch(`http://localhost:8080/api/orders/update-status/${id}?status=${encodeURIComponent(newStatus)}`, {
              method: "PUT"
            });
